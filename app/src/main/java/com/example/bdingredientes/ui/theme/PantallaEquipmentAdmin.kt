...
@Composable
fun PantallaEquipmentAdmin(db : VMBD3, sf: ViewModelScaffold, navController: NavController, launcher: ActivityResultLauncher<Intent>) {
    ...
    Column {
        ...
        Row {
            Text(text =
            if (delete.value) {if (idioma.value) {
                "Click on an equipment to delete it, click the Delete button again to stop Delete Mode"
            } else "Haz click en un utensilio para borrarlo, haz click en el botón Eliminar otra vez para parar el modo de Eliminado"
            } else if (update.value) {
                if (idioma.value) {
                    "Click on an equipment to modify it"
                } else "Haz click en un utensilio para modificarlo"
            } else {
                if (idioma.value) {
                    "Here is all of the equipment"
                } else "Aquí están todos los utensilios"
