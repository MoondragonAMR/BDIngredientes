...
fun PantallaAñadir(sf : ViewModelScaffold = viewModel()) {
    ...
    nombre = nombreAdd
    var tipoAdd by remember { mutableStateOf("Topping") }
    tipo = tipoAdd
    var saborAdd by remember { mutableStateOf("sweet") }
    sabor = saborAdd
    if (!idioma.value) {
        saborAdd = "dulce"
    }
    ...
    juego3 = juegoAdd
    var parteAdd by remember { mutableStateOf("Pizza Topping") }
    parte2 = parteAdd
    if (!idioma.value) {
        parteAdd = "Topping de pizza"
    }
    numero = 0
